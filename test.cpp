...
//#include "jump_stack_call_prepare_x64.h"
#ifdef __x86_64__
...
void double_jump(int a, float b, doublewordtype c, long d, double e, const double f,int &g, const int& h,int* i, const int& j){
	jump_stack_call_safe(buf, sizeof(buf), t1, 1,2,3,4,5,6,++z, z, &z, z);
	cout << "OK" << endl;
	jump_stack_call_safe(buf, sizeof(buf), t3, a,b,c,d,e,f,g,h,i,j);
	cout << "OK" << endl;
	jump_stack_call_safe(ibuf,t2, a,b,c,d,e,f,g,h,i,j );
	cout << "OK" << endl;
	jump_stack_call_safe(ibuf, t1, 1,2,3,4,5,6,++z, z, &z, z );
	cout << "OK" << endl;
	jump_stack_call_safe(buf, bb );
	cout<< "End" << endl;
}
...
int main(){
	...
	cout << "endl" << endl;			
	jump_stack_call(buf, sizeof(buf) , bb );
	jump_stack_call(buf, bb );
	jump_stack_call( buf, &bb1, z );
	jump_stack_call(buf, bb2, z, 134 );
	//call_with_stack_exp_class<void>::call_with_stack_exp(z, 134,  buf + sizeof(buf) - 8, bb2, bb1);
	t1(10000000000,2,3,4,5,6,++z, z, &z, z);
	jump_stack_call( buf, sizeof(buf), t1, 10000000000,2,3,4,5,6,++z, z, &z, z);
	jump_stack_call(buf ,t0, 1,2,3,4,5,6,++z, z, &z, z);
	jump_stack_call(buf ,t1 ,1,2,3,4,5,6,++z, z, &z, z);
	jump_stack_call(buf ,t8 ,1,2,3,4,5,6,++z, z, &z, z);
	jump_stack_call(buf ,sizeof(buf), t2, 1,2,3,4,5,6,++z, z, &z, 5 );
	jump_stack_call(buf ,t2, 1,2,3,4,5,6,++z, z, &z, 5);
	char *d = (char*)malloc(sizeof(buf));
	jump_stack_call(d, sizeof(buf), bb );
	jump_stack_call(d, sizeof(buf), t1 , 1,2,3,4,5,6,++z, z, &z, z );
	jump_stack_call(d, sizeof(buf), bb );
	free(d);
	jump_stack_call_safe(buf, sizeof(buf), double_jump, 1,2,3,4,5,6,++z, z, &z, z);
	jump_stack_call_safe(buf, double_jump, 1,2,3,4,5,6,++z, z, &z,z);
	jump_stack_call_safe(buf, double_jump1, 1,2,3,4,5,6,++z, z, &z, z);
	...
	jump_stack_call(buf , t3, 1,2,3,4,5,6,++z, z, &z, z) ;
	jump_stack_call_safe(buf, t4, 1,2,3,4,5,6,++z, z, &z, z);
	
	jump_stack_call(8*1024, t1, 1,2,3,4,5,6,++z, z, &z, z );
	
	cout << jump_stack_call(buf, t3, 1,2,3,4,5,6,++z, z, &z, z ) << endl;
	...
	jump_stack_call(buf, &printf, "%d%d%p%f%d\n", 1, z, &z, (double)z, z+1);
	return 0;
}
